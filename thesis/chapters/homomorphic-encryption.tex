...
\begin{definition}{Natural Projection $\underline{\pi}$}{pi-transform}
  Let $T$ be a mulitplicative subgroup of $\Z_d^*$ with $\Z_d^* / T = \{+1, -1\} = \{1T, -1T\}$, then the natural projection $\underline{\pi}: H \mapsto \C^{N/2}$ is defined as
...
The exact structure of $T$ is given by $\Z_d^* / T = \{\pm 1\}$ with $+1$ and $-1$ denoting left cosets of $T$.
