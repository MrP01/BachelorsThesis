...
For two tuples $(\cdot, \cdot)$ defined over the same group, denote their element-wise addition as $(\cdot, \cdot) + (\cdot, \cdot)$ and element-wise multiplication by a scalar $u$ as $u \cdot (\cdot, \cdot)$.
...
\begin{definition}{Discretisation to an element of $\underline{\sigma}(R)$}{}
  ...
  Note that $\underline{\rho}\inv$ is given directly because all elements of its domain are already elements of its image. Similarly, $\underline{\rho_\delta}\inv(\vec{y}) = \delta\inv \cdot \vec{y}$.
\end{definition}
...
\begin{definition}{The CKKS Scheme}{ckks-scheme}
  ...
  From the original CKKS paper, choose
  $\chi_{key} = \mathcal{H}WT$,
  $\chi_{error} = \mathcal{DG}$
  and $\mathcal{ZO}$ a distribution over ....
  \vspace{0.4cm}
  ...
  \begin{tblr}{Q[l,h]Q[l,h,\textwidth - 3.5cm]}
    ...
    \cryptop{Encrypt}$(\vec{p}, m)$ & {
        Let $(b,a) = \vec{p}$, $u \leftarrow \mathcal{ZO}$, $\mu_1, \mu_2 \leftarrow \chi_{error}$,
