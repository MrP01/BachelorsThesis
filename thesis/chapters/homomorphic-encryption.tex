...
% TODO: exaktere Einführung von \mathcal{E} (Definition)
...
\todo{Eine kleine historical introduction hierzu? Bootstrapping erwähnen..?}
% Roman: Just mention it shortly as I do in my papers:
% The first fully HE scheme was introduced by gentry, based on lattices, encryption introduces noise, he operations increase noise, bootstrapping is evaluating the decryption circuit homomorphically -> resets noise. Followup schemes improve his blueprint
...
\todo{Ausführlichere Beschreibung..?}
% Roman: I think you have never mentioned Galois rotations before...
% Maybe start this section with mentioning that in CKKS one can encode a vector of n/2 elements into one polynomial and addition/multiplication then correspond to elementwise vector addition/multiplications. Furthermore, these vectors can be rotated using galois automorphism.
...
\end{figure}
\todo{Schematik an Notation anpassen}
...
\todo{Obiges Theorem kurz zeigen (ausrechnen)}

\textit{Microsoft SEAL} \parencite{seal-4.0} implements the scheme, enabled using \cpp{seal::scheme_type::bfv}.
...
\todo{Obiges soll Motivation hinter CKKS zeigen.. fertig schreiben}
% Roman: The main motivitaion behind CKKS is that we have a homomorphic rounding operation which allows to reduce the scaling factors after mulitplication
...
\begin{definition}{Discretisation to an element of $\underline{\sigma}(R)$}{}
  Using one of several round-off algorithms (cf. \cite{2013-rlwe-toolkit}), given an element of $H$, define a rounding operation $\underline{\rho}: H \mapsto \underline{\sigma}(R)$ that maps an $\vec{h} \in H$ to its closest element in $\underline{\sigma}(R)$, also denoted as
  $$\underline{\rho}(\vec{h}) = \lfloor \vec{h} \rceil_{\underline{\sigma}(R)}\,.$$
  Further let $\underline{\rho_\delta}(\vec{h}) = \lfloor \delta \cdot \vec{h} \rceil_{\underline{\sigma}(R)}$ denote the same rounding operation but with prior scaling by a scalar factor $\delta$.
...
Note that for choosing a 'close' element $\vec{g} \in H$, we must first introduce a sense of proximity, in this case done by the $l_\infty$-norm $||\vec{g} - \vec{h}||_\infty$ of the difference between $\vec{h} \in H$ and $\vec{g}$.
...
\end{figure}
\todo{Schematik an CKKS anpassen}
...
\textit{Microsoft SEAL} \parencite{seal-4.0} implements the scheme, enabled using \cpp{seal::scheme_type::ckks}.
