...
As we can see, the term homomorphic encryption stems from the ability to perform computations on encrypted data while ensuring the same results are obtained when the same operations are applied to the original data.
...
\begin{definition}{Discretisation to an element of $\underline{\sigma}(R)$}{}
  Using one of several round-off algorithms (c.f. \cite{2013-rlwe-toolkit}), given an element of $H$, define a rounding operation $\underline{\rho}: H \mapsto \underline{\sigma}(R)$ that maps an $\vec{h} \in H$ to its closest element in $\underline{\sigma}(R)$, also denoted as
...
\begin{table}[H]
  ...
  \begin{tblr}{rll}
    ...
    $\lambda$ & $\in \R$ & Security parameter \\
    $d$ & $\in \N$ & Index of the cyclotomic polynomial used in $R$ \\
    $P$ & $\in \Z$ & \todo{Hmmm... wie sieht das genau aus?} \\ % TODO
