...
\begin{table}[H]
  ...
  \begin{tblr}{rll}
    ...
    $\vec{c}$ & $\in R^2$ & Ciphertext \\
    $\overline{\vec{c}}$ & $\in R^3$ & Slightly larger ciphertext resulting from multiplication \\
  \end{tblr}
...
We first notice that by \name{Lagrange}'s theorem on finite groups, the number of elements in $T$ is exactly $N / 2$ since $$\frac{|\Z_d^*|}{|T|} = |\{\pm 1\}| \Leftrightarrow \frac{N}{|T|} = 2 \Leftrightarrow |T| = \frac{N}{2}$$ leading to $\underline{\pi}(\mathcal{H}) \subseteq \C^{N/2}$.
...
\begin{figure}[H]
  ...
  \caption{Schematic overview of CKKS, adapted from \cite{2020-cryptotree}. Plain vectors $\vec{z} \in \C^{N/2}$ are encoded into plaintext polynomials $m = \cryptop{CKKS.Encode}(\vec{z})$, encrypted to the ciphertext $\vec{c} = \cryptop{CKKS.Encrypt}(\vec{p}, m)$ using the public key, operated on using a combination of \cryptop{CKKS.{Add, Mult, ReLin, ReScale}} ciphertext operations and finally decrypted and decoded to a new $\tilde{\vec{z}} = \cryptop{CKKS.Decode}(\cryptop{CKKS.Decrypt})(\tilde{\vec{c}})$ using the secret key.}
