...
\begin{proof}
  Similar to the BFV scheme proof (\autoref{thm:bfv-enc-is-homomorphic}), we aim show that two input vectors $\vec{z}, \vec{z}' \in \C^{N/2}$ can be encoded, encrypted and added - and finally decrypted back to $\overline{\vec{z}} = \vec{z} + \vec{z}'$.
  ...
  The rest follows from \autoref{thm:bfv-enc-is-homomorphic} as encryption and addition of the BFV scheme are identical.
  All in all for \cryptop{CKKS},
  $$\cryptop{Decode}(
    \cryptop{Decrypt}(s,
    \cryptop{Add}(
