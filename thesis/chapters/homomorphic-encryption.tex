...
To summarise the parameters and variables, a brief overview of all used symbols is provided in \cref{tab:bfv-symbols}.
\begin{table}[H]
  \centering
...
Note that for choosing a 'close' element $\vec{g} \in \mathbb{H}$, we must first introduce a sense of proximity, in this case done by the $l_\infty$-norm $||\vec{g} - \vec{h}||_\infty$ of the difference between $\vec{h} \in \mathbb{H}$ and $\vec{g}$.
...
To summarise the parameters and variables, a brief overview of all used symbols is provided in \cref{tab:ckks-symbols}.
\begin{table}[H]
  \centering
...
\begin{proof}
  Similar to the BFV scheme proof (\cref{thm:bfv-enc-is-homomorphic}), we aim show that two input vectors $\vec{z}, \vec{z}' \in \C^{N/2}$ can be encoded, encrypted and added - and finally decrypted back to $\overline{\vec{z}} = \vec{z} + \vec{z}'$.
