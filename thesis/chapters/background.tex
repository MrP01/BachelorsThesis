...
\begin{definition}{Polynomial Ring over $\Z$}{poly-ring}
  ...
  \begin{itemize}
    \item In their coefficient representations $\vec{p} = \{p_0, p_1, p_2, ...\}$ and $\vec{q} = \{q_0, q_1, q_2, ...\}$,
...
\begin{definition}{The $n$\textsuperscript{th} roots of unity}{nth-roots-of-unity}
...
\begin{definition}{Cyclotomic Polynomial}{cyclotomic-poly}
  Given the $n$\textsuperscript{th} roots of unity $\{x_k\}$, we can define the $n$\textsuperscript{th} cyclotomic polynomial as
  $$\Phi(x) = \prod_{k=1}^{\varphi(n)} (x - x_k)$$
