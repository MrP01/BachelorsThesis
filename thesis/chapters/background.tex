...
\begin{itemize}
  \item In their coefficient representations $\vec{p} = (p_j)_{j\in\N} = (p_0, p_1, p_2, ...)$ (which are sequences) and $\vec{q} = (q_j)_{j\in\N} = (q_0, q_1, q_2, ...)$, an addition of two polynomials $p, q \in \Z[X]$ is equivalent to the element-wise addition of their coefficient sequences
        ...
        \end{align*}
        which indeed satisfies the additive \hyperref[def:ring]{ring axioms} due to the existing structure of the underlying field $\C$.
  \item The multiplication operation can be defined using a discrete convolution of the coefficient vectors
        ...
        Again, this generally applicable approach satisfies the multiplicative \hyperref[def:ring]{ring axioms} and even satisfies commutativity due to the existing structure of the underlying field $\C$ and the symmetry of convolutions.
\end{itemize}
...
\begin{lemma}{The $n$\textsuperscript{th} roots of unity}{nth-roots-of-unity}
  For some integer $n \in \N$, the $n$ complex roots $x_1, x_2, ..., x_n \in \C$ of unity can be found as $$x_k = e^{2\pi i \frac{k}{n}} \quad k \in \{1, 2, ..., n\}$$ with $i$ the imaginary unit.
  Confer \autoref{fig:nth-roots-of-unity}.
  Using \name{Euler}'s identity, their real and imaginary components can be explicitly found as $x_k = \cos(2\pi \frac{k}{n}) + i \sin(2\pi \frac{k}{n})$.
...
\end{lemma}
Due to the fact that for any $k, l \in \Z$, their product $x_k \cdot x_l$ is also a root of unity, and $x_{k+jn} = x_k \; \forall j \in \Z$, they clearly comprise a cyclic Abelian group over the complex numbers $\C$ under multiplication with (for instance) the first root $x_1 = e^{2\pi i \frac{1}{n}}$ as its generator.

\begin{figure}
  \centering
  \inputtikz{figures/nth-roots-of-unity}
  \caption[]{The 5\textsuperscript{th} roots of unity visualised on the complex plane. Obviously, they all lie on the unit circle $|z| = 1$, motivating the name of cyclotomic, 'circle-cutting', polynomials, whose roots cut the unit circle into multiple sectors.}
...
Examples include collision resistant hash functions, signatures, pseudorandom functions or even Regev's public-key cryptosystem that is based on LWE (which is reduced to the other lattice problems) \parencite{2016-decade-of-lattice}.

\begin{figure}
  \centering
  \inputtikz{figures/lattice}
  \caption{Illustration of a standard lattice $\lat$ over the integers $\Z$
    with two basis vectors $\vec{b}_1$ and $\vec{b}_2$, cf. \autoref{def:lattice}.
...
\todo{LWE and RLWE comparison}
% TODO: maybe mention here, that in LWE based cryptosystems, the public key is m LWE samples, i.e., (A, b) where A is a m x m matrix and b a m vector. For RLWE it is then just (a,b) where a and b have b coefficients. Hence, keys are smaller in RLWE.
% Futhermore, RLWE samples can be translated into equivalent LWE samples. in the case above, the matrix A is build from the polynomial a and then LWE = RLWE. When we derive security for RLWE we take secure parameters for this equivalent LWE representation. Thereby we "hope" that building A from the poly a does not introduce any security flaw :)
