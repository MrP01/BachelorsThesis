...
One major problem of the diagonal method is that it requires many Galois rotations of $\vec{x}$, which is slow and also causes a large error after too many consecutive rotations (\cref{fig:rotation-error}).
...
\begin{theorem}{Hybrid Method}{hybrid-method}
  ...
  $$M \vec{x} = (y_i)_{i \in \Z/s\Z} \;\text{with}\; \vec{y} = \sum_{k=0}^{t / s} \rot_{ks}\bigg(\sum_{j=0}^s \diag_j(M) \, \rot_j(\vec{x})\bigg) \,.$$
\end{theorem}

The hybrid method almost always outperforms the na\"ive and diagonal methods \parencite{2018-gazelle}.

To exemplarily discuss the implementation of an \gls{he} algorithm, consider the following function that uses the matrix multiplication method described above:
\begin{minted}{cpp}
void DenseLayer::matmulHybrid(seal::Ciphertext &in_out, const Matrix &matrix, seal::GaloisKeys &galois_keys, seal::CKKSEncoder &encoder, seal::Evaluator &evaluator) {
  size_t in_dim = matrix.shape(0), out_dim = matrix.shape(1);

  // diagonal method preparation
  std::vector<seal::Plaintext> diagonals = encodeMatrixDiagonals(matrix, encoder);
  ...
  in_out = sum;  // and we arrive at the final result
