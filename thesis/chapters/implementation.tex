...
The network implemented in our demonstrator, trained using the \textit{Tensorflow} machine learning framework in Python \footnote{\url{https://www.tensorflow.org/}}, has the following layer structure (also confer \autoref{fig:neural-network}):
\begin{align*}
...
In order to reduce the number of rotations required, one can make use of the \textit{Babystep-Giantstep} optimisation as described in \cite{2018-faster-helib}, which works as follows:
