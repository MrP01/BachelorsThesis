...
\gls{mnist} is one of the most commonly used datasets in \glsdesc{ml}, featured in an abundant number of tutorials and showcases, all in all it is very well studied.
...
\label{sec:matmul}  % TODO: x \in R^s oder R^t
...
\begin{enumerate}
  ...
  \item multiply each encoded row with the encrypted vector using \texttt{Evaluator.multiply\_plain()} to obtain the ciphertext vector $\vec{y_i} \in \R^s$ for row $i$.
